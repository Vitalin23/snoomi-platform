...
def _build_onboarding_progress():
    ...
    if channel_ids:
        step2_done = (
            ChannelTopic.query.filter(
                ChannelTopic.channel_id.in_(channel_ids),
                ChannelTopic.is_active.is_(True),
            )
            .limit(1)
            .first()
            is not None
        ...
        if not step2_done:
            settings_rows = (
                ChannelSetting.query.filter(ChannelSetting.channel_id.in_(channel_ids))
                .with_entities(ChannelSetting.topics)
                .all()
            )
...
def inject_common_template_context():
    return {
        ...
        "onboarding_progress": _build_onboarding_progress(),
    }
